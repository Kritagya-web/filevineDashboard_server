...
def get_projects_by_type(code: str, limit: int = 200) -> list[int]:
    projects = []
    offset = 0
    page_sz = 100  # Max page size FileVine allows
    attempts = 0
    max_attempts = 5  # Prevent infinite loops
    
    while len(projects) < limit and attempts < max_attempts:
        try:
            headers = get_dynamic_headers()
            resp = requests.get(
                ...
                params={
                    "offset": offset,
                    "limit": page_sz,
                    "projectTypeCode": code  # Server-side filtering
                },
                timeout=30
            )
            ...
            # All returned items should already be filtered by projectTypeCode
            for pj in items:
            ...
            print(f"⚠️ Error fetching projects (attempt {attempts + 1}): {e}")
            attempts += 1
...
if __name__ == "__main__":
    ...
    project_ids = get_projects_by_type("LOJE 2.0", limit=200)
